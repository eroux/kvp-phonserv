...
def test_kangki_lodro_one_by_one():
    ...
    """
    assert_equal_phonetics(tibetan, expected, mode="one", schema="kvp")
...
def test_kangki_lodro_two_by_two():
    ...
    """
    assert_equal_phonetics(tibetan, expected, mode="two", schema="kvp")

def test_kangki_lodro_word_by_word():
    expected = """
        gang gi lodrö dribnyi trin dral nyi tar namdak rabsalwé
        jinyé dön kün jizhin zik chir nyi kyi tugkar legbam dzin
        gangdak sipé tsönrar ma rik mün tum dugngal gyi zirwé
        dro tsok kün la bu chik tar tsé yenlak druk chü yang den sung
        druk tar cher drok nyönmong nyi long lé kyi chagdrok dröl dzé ching
        ma rik münsel dugngal nyugu jinyé chöd dzé raldri nam
        dö né dak ching sachü tarsön yönten lüdzok gyalsé tuwö ku
